...
public class InGameManager : MonoSingleton<InGameManager>
{
    ...
    {        
        ...
        PlayerData playerData1 = new(UserDataManager.NickName, UserDataManager.UserData.selectIndex, true, 0, UserDataManager.Money, UserDataManager.UserData.gage);

        CharacterTableData characterTableData = TableDataManager.Instance.TableCharacterData.GetLevelTableDataByLevel(UserDataManager.Level);
        ...
        if (_multiple)
        {
            if (!_useAds)
            {
                if (WinMultipleCount == 3)
                {
                    ...
                    _ = NetworkManager.Instance.SendItemLog("GoldWinMultiple", -gold);
                }
                else
                {
    ...
    public void SendReqNextLevel()
    {
        ...
        CharacterTableData data = TableDataManager.Instance.TableCharacterData.GetLevelTableDataByLevel(UserDataManager.Level);
    ...
    public void SendReqFireMatchConfirm(bool _multiple = false, bool _useGold = false)
    {
        ...
        else
        {
            ...
            if (_useGold)
            {
                ...
                _ = NetworkManager.Instance.SendItemLog("GoldFireMiltiple", -ConfigData.FireMutilpleGold);
            }
    ...
    IEnumerator OnResultCourotine(bool _checkGukjun = true, float _delay = 0.0f)
    {
        ...
        UserDataManager.SetResult(CurrentTurnSlotIndex == 0, _resultEvent.netResultInfo.reward, _resultEvent.netResultInfo.score * _resultEvent.netResultInfo.multiPle, CurrentTurnPlayer.goCount);
