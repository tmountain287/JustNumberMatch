...
public sealed class BossStageSessionController : GameSessionController
{
    ...
    public override void ReadySession(IStageSequence sequence)
    {
        ...
        // 난이도별 제한시간(ms) 예: Easy 60초, Normal 45초, Hard 30초 등
        long limitMs = ConfigData.BossTimeDic[difficultyType];// ConfigData.BossTimeDic[difficultyType];
        ...
        GameMgr.Instance.GameUI.GameTopUI.SliderTimer.InitTimer(limitMs, ConfigData.BossTimeStarDic[difficultyType]);
    ...
    private void StageComplete()
    {
        ...
        if (StageTableData.id < TableDataManager.Instance.TableStageData.StageTableDataDic[StageTableData.difficultyType][^1].id)
        {
            nextAction = () =>
            {
                StageTableData data = TableDataManager.Instance.TableStageData.GetTableData(StageTableData.difficultyType, StageTableData.id + 1);
                GameMgr.Instance.StartStageMode(data);
            };
        }
