...
public class FirestoreDiag : MonoSingletonDont<FirestoreDiag>
{
    ...
    public async Task InitializeAsync(CancellationToken ct = default)
    {
        ...
        // EnableNetworkAsync는 내부적으로 Future를 만들기 때문에, await 없이 두면 종료 시점에 남아 경고가 날 수 있음.
        await db.EnableNetworkAsync()
            .TimeoutAfter(defaultTimeout, ct);
    }
    ...
    CancellationToken externalCt = default)
    {
        ...
        // isOverwrite + FieldValue.Delete 는 merge/update 에서만 유효. 첫 SetAsync(merge 없음)에 넣으면
        // "FieldValue.delete() can only be used with update() and set() with SetOptions.merge()" 발생.
        var dataWithOverwriteCleanup = new Dictionary<string, object> {
        { "us_id", us_id },
        { "device_id", device_id },
        { "record", record },
        ...
        { "isOverwrite", FieldValue.Delete },  // 기존 문서 merge 시에만 사용
    };

        var dataFirstCreate = new Dictionary<string, object> {
        { "us_id", us_id },
        { "device_id", device_id },
        { "record", record },
        { "updatedAt", FieldValue.ServerTimestamp },
        { "forceConflict", forceConflict },
        { "checkDevice", checkDevice },
        // create 규칙 hasOnly 에도 isOverwrite 없음 — 신규 문서엔 필드 자체를 두지 않음
    };
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++)
        {
            ...
            try
            {
                ...
                if (!snap.Exists)
                {
                    // 첫 저장: create 경로 (FieldValue.Delete 금지)
                    await doc.SetAsync(dataFirstCreate).TimeoutAfter(defaultTimeout, cts.Token);
                }
                else
                {
                    // 이후 저장: merge (여기서만 isOverwrite 삭제 sentinel 허용)
                    await doc.SetAsync(dataWithOverwriteCleanup, SetOptions.MergeAll).TimeoutAfter(defaultTimeout, cts.Token);
                }
