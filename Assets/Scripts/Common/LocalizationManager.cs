    ...
    public class LocalizationManager : MonoSingletonDont<LocalizationManager>
    {
        ...
        public LocalType CurrentLocalType
        {
            get
            {
                ...
                return (LocalType)currentLanguageIndex;
            }
        }

        public string GetText(string _entryKey, LocalUIType _type = LocalUIType.Normal)
        {
            if (string.IsNullOrEmpty(_entryKey))
                return string.Empty;

            TableDataManager tm = TableDataManager.Instance;
            if (tm == null || tm.TableLocalizationData == null)
                return string.Empty;

            try
            {
                return tm.TableLocalizationData.GetLocalString(_type, _entryKey, CurrentLocalType);
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[LocalizationManager] GetText failed key={_entryKey} type={_type}: {ex.Message}");
                return string.Empty;
            }
        }
