...
#pragma warning restore 0649
        ...
        private Vector2[] startPositions = new Vector2[3];
        ...
        private void Update()
		{
            ...
            if (Input.touchCount == 3)
            {
                if (!tracking)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        startPositions[i] = Input.GetTouch(i).position;
                    }
                }
                else
                {
                    bool allMovedDown = true;
                    for (int i = 0; i < 3; i++)
                    {
                        Vector2 delta = Input.GetTouch(i).position - startPositions[i];
                        if (delta.y > -50f) // 아래로 스와이프한 거리 (조절 가능)
                        {
                            allMovedDown = false;
                            break;
                        }
